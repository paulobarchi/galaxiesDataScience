...
print '\nTotal: ' + str(len(df2))
print '\nTotal classified (E or S): ' + \
	str(len(df2[(df2['class'] == 'E')]) + len(df2[(df2['class'] == 'S')]))
...
for metric in metrics:
	...
	# merged.loc[(merged[metric].isnull()), metric] = 9999.99
...
problems = ( (merged['G2'] == 9999.99) | (merged['H'] == 9999.99) | \
	(merged['C'] == 9999.99) | (merged['A'] == 9999.99) | (merged['S'] == 9999.99) )
...
print '\nTotal classified (E or S): ' + \
	str(len(merged[(merged['class'] == 'E')]) + len(merged[(merged['class'] == 'S')]))
