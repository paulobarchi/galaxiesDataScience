...
def forward_pass(images, net, transformer, batch_size=1, log):
    """
    ...
    for chunk in [caffe_images[x:x+batch_size] for x in xrange(0, len(caffe_images), batch_size)]:
        ...
        print('Processed %s/%s images ...' % (len(scores), len(caffe_images)), file=log)
        # print 'Processed %s/%s images ...' % (len(scores), len(caffe_images))

    return scores

def read_labels(labels_file, log):
    """
    ...
    """
    if not labels_file:
        print('WARNING: No labels file provided. Results will be difficult to interpret.', file=log)
...
def classify(caffemodel, deploy_file, image_files,
        mean_file=None, labels_file=None, use_gpu=True, log):
    """
    ...
    labels = read_labels(labels_file, log)

    # Classify the image
    classify_start_time = time.time()
    scores = forward_pass(images, net, transformer, log)
    print('Classification took %s seconds.' % (time.time() - classify_start_time,), file=log)
    ...
    for index, classification in enumerate(classifications):
        print('{:-^80}'.format(' Prediction for %s ' % image_files[index]), file=log)
        # print '{:-^80}'.format(' Prediction for %s ' % image_files[index])
        for label, confidence in classification:
            print('{:9.4%} - "{}"'.format(confidence/100.0, label),file=log)
...
if __name__ == '__main__':
    ...
    classify(args['caffemodel'], args['deploy_file'], image_files,
            args['mean'], args['labels'], not args['nogpu'], log)

print('Script took %s seconds.' % (time.time() - script_start_time), file = log)
